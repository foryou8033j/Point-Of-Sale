...
public class CashPaymentLayoutController {
	...
	if (payMoney == 0) {
		...
		if (var < 0)
		    new SimpleAlert(stage, AlertType.INFORMATION, "거스름돈", "거스름돈 " + tableData.getResultPay() + " 원 반환")
			    .showAndWait();

		// 영수증 기록을 남긴다.
		mainApp.getReceipts().addReceiptList(PAY_WAY.CASH, tableData, tableData.getResultPay());
	...
	/*
	 * if(payMoney >= tableData.getResultPay()) payMoney = tableData.getResultPay();
	 */
