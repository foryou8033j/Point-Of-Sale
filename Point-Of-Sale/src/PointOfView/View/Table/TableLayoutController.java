...
public class TableLayoutController implements Initializable {
    ...
    /** 컴포넌트 정의 **/
    @FXML
    Button btnPrevToTitle;
    @FXML
    Button btnTableManagement;
    @FXML
    Button btnMenuManagement;
    @FXML
    Button btnReceptionManagement;
    @FXML
    Button btnStasticsManagement;

    @FXML
    Label lbnRestaurant;
    @FXML
    Label lbnCurrentTime;
	...
	for (int i = 0; i < size; i++) {
		...
		if (removeTableList.contains(tables.get(index))) {
		    pane.setStyle("-fx-border-color: #FF0000; " + "-fx-border-width: 1.5;" + "-fx-border-radius: 15;"
			    + "-fx-background-radius: 16.4, 15;" + "-fx-background-color: #FFEEE9");
		} else if (originalTableData != null && originalTableData.equals(tables.get(index))) {
		    pane.setStyle("-fx-border-color: #00FF00;" + "-fx-border-width: 1.5;" + "-fx-border-radius: 15;"
			    + "-fx-background-radius: 16.4, 15;" + "-fx-background-color: #EEFFE9");
		} else {
	...
	try {
	    ...
	    // 디버그를 위한 메뉴 추가
	    /*
	     * tableData.addMenu(mainApp.getDataManagement().getMenues().getMenuItems().get(
	     * 0));
	     * tableData.addMenu(mainApp.getDataManagement().getMenues().getMenuItems().get(
	     * 1));
	     * tableData.addMenu(mainApp.getDataManagement().getMenues().getMenuItems().get(
